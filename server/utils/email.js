...
export const sendApprovalEmail = async (booking, plot_number, layout_name, location) => {
  try {
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS && process.env.SMTP_HOST) {
        ...
        // Local Demo Mode: Print instead of connecting to a remote server
        console.log("-----------------------------------------");
        console.log("📧 [DEMO MODE] EMAIL MOCK");
        console.log(`To: ${booking.customer_email || 'test-customer@example.com'}`);
        console.log(`Subject: [CONFIRMED] Site Visit for Plot ${plot_number}`);
        console.log(`Message: Site visit for ${booking.customer_name} at ${layout_name} is approved.`);
        console.log("-----------------------------------------");
    }
